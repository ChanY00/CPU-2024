...
# 엑셀 파일 불러오기
file_path = 'C:\\Users\\kseon\\CPU\\캠퍼스 유니버시아드\\코드\\CPU_data_small.xlsx'  # 파일 경로 지정
...
# 대표출원인 빈도수 계산 및 상위 10개 필터링
applicant_freq = df['대표출원인'].value_counts().head(10)
top_applicants = applicant_freq.index.tolist()
df_top = df[df['대표출원인'].isin(top_applicants)]
print("상위 10개 대표출원인 필터링 완료")

# 연도별, 기술별로 그룹화하고 키워드 빈도분석
keyword_freq = df_top.groupby(['대표출원인', '중분류'])['발명의 명칭'].apply(lambda x: ' '.join(x)).apply(lambda x: pd.Series(nltk.FreqDist(word_tokenize(x.upper())))).unstack().fillna(0)
print("키워드 빈도 분석 완료")
keyword_freq_bigrams = df_top.groupby(['대표출원인', '중분류'])['발명의 명칭_bigrams'].apply(lambda x: ' '.join(x)).apply(lambda x: pd.Series(nltk.FreqDist(word_tokenize(x.upper())))).unstack().fillna(0)
print("바이그램 빈도 분석 완료")
...
save_to_excel_in_chunks(keyword_freq_transposed, "C:\\Users\\kseon\\CPU\\캠퍼스 유니버시아드\\코드\\제1출원인\\CPU_person2_freq_small")
print("키워드 빈도 분석 결과 저장 완료")

# 바이그램 빈도 분석 결과를 여러 개의 EXCEL 파일로 저장
save_to_excel_in_chunks(keyword_freq_bigrams_transposed, "C:\\Users\\kseon\\CPU\\캠퍼스 유니버시아드\\코드\\제1출원인\\CPU_person2_bigram_freq_small")
print("바이그램 빈도 분석 결과 저장 완료")

# 대표출원인 빈도수 결과를 EXCEL 파일로 저장
applicant_freq.to_excel("C:\\Users\\kseon\\CPU\\캠퍼스 유니버시아드\\코드\\제1출원인\\CPU_applicant2_freq_small.xlsx")
print("대표출원인 빈도수 결과 저장 완료")
