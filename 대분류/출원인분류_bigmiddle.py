...
# 엑셀 파일 불러오기
file_path = "C:\\workstation\\CPU\\CPU-2024\\대분류\\CPU_data_big_entire.xlsx"  # 파일 경로 지정
...
applicant_freq = df['제1출원인'].value_counts().head(11)
...
save_to_excel_in_chunks(keyword_freq_transposed, "C:\\workstation\\CPU\\CPU-2024\\대분류\\대분류_제1출원인_키워드 분석.xlsx")
print("키워드 빈도 분석 결과 저장 완료")

# 바이그램 빈도 분석 결과를 여러 개의 EXCEL 파일로 저장
save_to_excel_in_chunks(keyword_freq_bigrams_transposed, "C:\\workstation\\CPU\\CPU-2024\\대분류\\대분류_출원인 바이그램 분석.xlsx")
print("바이그램 빈도 분석 결과 저장 완료")

# 제1출원인 빈도수 결과를 EXCEL 파일로 저장
applicant_freq.to_excel("C:\\workstation\\CPU\\CPU-2024\\대분류\\대분류_제1출원인 분석.xlsx")
