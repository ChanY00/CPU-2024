...
# 엑셀 파일 불러오기
file_path = 'C:\\Users\\dnjsr\\Desktop\\특허 데이터 분석 참고 자료\\코드\\데이터\\디지털웰에이징_초거대인공지능.xlsx'  # 파일 경로 지정
df = pd.read_excel(file_path)

# 필요한 열에 대한 전처리 수행
df['요약(원문)'] = df['요약(원문)'].astype(str)
df['요약(원문)'] = df['요약(원문)'].apply(lambda x: x.upper())  # 대문자 변환 혹은 다른 필요한 처리
...
# '요약(원문)' 열에 적용하여 특수 문자 제거
df['요약(원문)'] = df['요약(원문)'].astype(str).apply(remove_special_characters)
...
# 불용어 제거 및 대체 작업 적용
df['요약(원문)'] = df['요약(원문)'].astype(str).apply(remove_stopwords)
...
# 여러분의 DataFrame('df')을 가정하고 있는데, 여기서 '요약(원문)' 컬럼을 가정합니다.
# 해당 컬럼의 각 단어를 lemmatize하여 다시 컬럼에 할당하는 예시입니다.
df['요약(원문)'] = df['요약(원문)'].astype(str).apply(lambda x: ' '.join([lemmatizer.lemmatize(word) for word in x.split()]))
...
# DataFrame의 요약(원문) 컬럼에 적용
df['요약(원문)'] = df['요약(원문)'].astype(str).apply(lemmatize_text)
...
# DataFrame의 '요약(원문)' 열에 적용하여 명사만 추출
df['요약(원문)'] = df['요약(원문)'].astype(str).apply(extract_noun_words)
...
for key, value in replacements.items():
    df['요약(원문)'] = df['요약(원문)'].str.replace(key, value, case=False)
...
# '요약(원문)' 열에 적용하여 bigram 생성
df['요약(원문)_bigrams'] = df['요약(원문)'].astype(str).apply(create_bigrams)

# 연도별, 기술별로 그룹화하고 키워드 빈도분석
keyword_freq = df.groupby(['기술', df['출원일'].dt.year])['요약(원문)'].apply(lambda x: ' '.join(x)).apply(lambda x: pd.Series(nltk.FreqDist(word_tokenize(x.upper())))).unstack().fillna(0)

# bigram에 대한 빈도분석
keyword_freq_bigrams = df.groupby(['기술', df['출원일'].dt.year])['요약(원문)_bigrams'].apply(lambda x: ' '.join(x)).apply(lambda x: pd.Series(nltk.FreqDist(word_tokenize(x.upper())))).unstack().fillna(0)
...
merged_df.to_csv("C:/Users/dnjsr/Desktop/특허 데이터 분석 참고 자료/코드/biagram_키워드_빈도분석.csv")
